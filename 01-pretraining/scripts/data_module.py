# data_module.py - Data module (handles image+text and image-only datasets)
...
# get augmentation functions from all_datasets_transforms.py
...
class AllDatasetsDataModule(LightningDataModule):
    ...
    def __init__(self, 
                 ...
                 use_text=False, # whether to use text encoder
                 ):
    ...
    def setup(self, stage=None):
        ...
        # if using image and text, use MultisourcePatchImageTextDataset
        if self.use_text:
            if not self.prompt_jsons:
                raise ValueError('prompt_jsons must be provided if use_text=True.')

            self.train_ds = MultisourcePatchImageTextDataset(
            ...
            self.val_ds = MultisourcePatchImageTextDataset(
        ...
        # if using image only, use MultisourcePatchImageOnlyDataset
        else:
            self.train_ds = MultisourcePatchImageOnlyDataset(
                file_paths=train_files, 
                transforms=train_tf,
                use_sub_patches=self.use_sub_patches,
                base_patch_size=self.base_patch_size,
            ...
            self.val_ds = MultisourcePatchImageOnlyDataset(
                file_paths=val_files, 
                transforms=val_tf,
                use_sub_patches=self.use_sub_patches,
                base_patch_size=self.base_patch_size,
