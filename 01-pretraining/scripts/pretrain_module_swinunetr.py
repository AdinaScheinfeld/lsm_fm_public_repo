# pretrain_module_swinunetr.py - PyTorch Lightning module for pretraining with a SwinUNETR backbone (image+text or image-only)
...
# main module class for pretraining using a SwinUNETR backbone (3D)
class PretrainModuleSwinUNETR(pl.LightningModule):
    ...
    def __init__(self, config: dict):
        ...
        # *** SwinUNETR Encoders ***
        swinunetr_cfg = config["model"].get("swinunetr", {})
        feature_size = int(swinunetr_cfg.get("feature_size", 48))
        use_checkpoint = bool(swinunetr_cfg.get("use_checkpoint", True))
        ...
        self.teacher_encoder = SwinUNETR(
            ...
            out_channels=self.embed_dim,
            feature_size=feature_size,
            use_checkpoint=False,  # no need for gradient checkpointing on teacher (no backprop)
        )
        ...
        if self.use_text:
            ...
            # separate projection head for image features to be used in CLIP loss and alignment loss
            # (instead of using student features directly, to allow more flexibility in learning a joint embedding space)
    ...
    # function to encode images into embeddings using student or teacher encoder
    ...
    def log_image_and_embeddings(
        ...
        # get count of images logged so far this epoch
        ...
        # for image embeddings, apply projection head to normalized student features if using text, otherwise use normalized student features directly
    ...
    def shared_step(self, batch, batch_idx, is_train: bool = True):
        ...
        # log composite val loss for easier comparability across runs with different loss weights (only log at epoch level to reduce noise)
        if not is_train:
            val_loss_report = (
    ...
    # at the end of each training epoch, log images and embeddings to wandb
    ...
    # at the end of each validation epoch, log images and embeddings to wandb and update best metrics
