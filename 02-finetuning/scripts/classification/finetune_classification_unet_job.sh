#!/bin/bash
#SBATCH --job-name=finetune_cls_unet
#SBATCH --output=../../output/logs/finetune_cls_unet_%j.out
#SBATCH --error=../../output/logs/finetune_cls_unet_%j.err
...
# finetune_classification_unet_job.sh - SLURM script for UNet classification finetuning
...
echo "Starting LSM UNet classification finetuning..."
...
# path to config file
CONFIG_PATH="finetune_classification_unet_config.yaml"
...
# run finetuning
srun --label python finetune_classification_unet.py --config "$CONFIG_PATH"
...
echo "LSM UNet classification finetuning complete."
