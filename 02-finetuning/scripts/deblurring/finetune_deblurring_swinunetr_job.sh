#!/bin/bash
#SBATCH --job-name=finetune_deblur_swinunetr
#SBATCH --output=../../output/logs/finetune_deblur_swinunetr_%j.out
#SBATCH --error=../../output/logs/finetune_deblur_swinunetr_%j.err
...
# finetune_deblurring_swinunetr_job.sh - SLURM script for SwinUNETR deblurring finetuning
...
echo "Starting LSM SwinUNETR deblurring finetuning..."
...
# path to config file
CONFIG_PATH="finetune_deblurring_swinunetr_config.yaml"
...
# run finetuning
srun --label python finetune_deblurring_swinunetr.py --config "$CONFIG_PATH"
...
echo "LSM SwinUNETR deblurring finetuning complete."
