#!/bin/bash
#SBATCH --job-name=finetune_swinunetr
#SBATCH --output=../../output/logs/finetune_swinunetr_%j.out
#SBATCH --error=../../output/logs/finetune_swinunetr_%j.err
...
# finetune_segmentation_swinunetr_job.sh - SLURM script for SwinUNETR finetuning
...
echo "Starting LSM SwinUNETR finetuning..."
...
# path to config file
CONFIG_PATH="finetune_segmentation_swinunetr_config.yaml"
...
# run finetuning
srun --label python finetune_segmentation_swinunetr.py --config "$CONFIG_PATH"
...
echo "LSM SwinUNETR finetuning complete."
